...
__all__ = ['ResGroupFix', 'resgroupfix50', 'resgroupfix101',
           'resgroupfix152']


model_urls = {
    'resgroupfix50': 'https://drive.google.com/uc?export=download&id=1O9-pGasclSTY8jVB_ez1ANv-DTZ8Sgo9',
    'resgroupfix101': 'https://drive.google.com/uc?export=download&id=1oqTfG-zpiHr7hKYuxMbE2v10nF2zNSxO',
    'resgroupfix152': 'https://drive.google.com/uc?export=download&id=1-TveOWnOHdDZLrAsKZ2Xj6pFO25X1cIG',
}
...
def resgroupfix50(pretrained=False, **kwargs):
    """Constructs a ResGroupFix-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResGroupFix(ResGroupBlock, [3, 4, 6, 3], **kwargs)
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load(download_from_url(model_urls['resgroupfix50'],
                                                           root=default_cache_path)))
    return model


def resgroupfix101(pretrained=False, **kwargs):
    """Constructs a ResGroupFix-101 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResGroupFix(ResGroupBlock, [3, 4, 23, 3], **kwargs)
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load(download_from_url(model_urls['resgroupfix101'],
                                                           root=default_cache_path)))
    return model


def resgroupfix152(pretrained=False, **kwargs):
    """Constructs a ResGroupFix-152 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResGroupFix(ResGroupBlock, [3, 8, 36, 3], **kwargs)
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load(download_from_url(model_urls['resgroupfix152'],
                                                           root=default_cache_path)))
    return model
