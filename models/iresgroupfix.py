...
__all__ = ['iResGroupFix', 'iresgroupfix50', 'iresgroupfix101', 'iresgroupfix152']


model_urls = {
    'iresgroupfix50': 'https://drive.google.com/uc?export=download&id=12oeCmPhaID2s3Doay8ZXMehnaaWqa_ek',
    'iresgroupfix101': 'https://drive.google.com/uc?export=download&id=1A8MIGI1ZEIfaB1bn_NN4QkoHKGRGxW_i',
    'iresgroupfix152': 'https://drive.google.com/uc?export=download&id=1s-4zKwbwlAJruSm-OFYmw96_qew7F2nw',
}
...
def iresgroupfix50(pretrained=False, **kwargs):
    """Constructs a iResGroupFix-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = iResGroupFix(ResGroupBlock, [3, 4, 6, 3], **kwargs)
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load(download_from_url(model_urls['iresgroupfix50'],
                                                           root=default_cache_path)))
    return model


def iresgroupfix101(pretrained=False, **kwargs):
    """Constructs a iResGroupFix-101 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = iResGroupFix(ResGroupBlock, [3, 4, 23, 3], **kwargs)
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load(download_from_url(model_urls['iresgroupfix101'],
                                                           root=default_cache_path)))
    return model


def iresgroupfix152(pretrained=False, **kwargs):
    """Constructs a iResGroupFix-152 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = iResGroupFix(ResGroupBlock, [3, 8, 36, 3], **kwargs)
    if pretrained:
        os.makedirs(default_cache_path, exist_ok=True)
        model.load_state_dict(torch.load(download_from_url(model_urls['iresgroupfix152'],
                                                           root=default_cache_path)))
    return model
